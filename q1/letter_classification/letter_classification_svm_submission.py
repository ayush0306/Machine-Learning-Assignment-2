...
def SVM(train_data,
    ...
    if(degree == 0) :
        x = svm.SVC(kernel = kernel, C = C, gamma = gamma)
    else :
        x = svm.SVC(kernel = kernel, C = C, gamma = gamma, degree = degree)   
    x.fit(train_data, train_labels)
    train_predictions = x.predict(train_data)
    test_predictions = x.predict(test_data)
...
def calcMetrics(accMetric):
    acc = float(0.0)
    precision = float(0.0)
    recall = float(0.0)
    f1 = float(0.0)

    for metric in sorted(accumulated_metrics): 
        acc += float(metric[0])
        precision += float(metric[1])
        recall += float(metric[2])
        f1 += float(metric[3])
    
    average_accuracy = float(acc)/float(5.0)
    average_precision = float(precision)/float(5.0)
    average_recall = float(recall)/float(5.0)
    average_f1 = float(f1)/float(5.0)
...
if __name__ == '__main__':
    ...
    #             accuracy,precision,recall,f1 = calcMetrics(accumulated_metrics)
