...
def kernelize(X, Y):
    ...
    # YOUR CODE HERE
    # print(X.shape)
    a = np.sum(np.multiply(X, X), axis=1)   #Kernel = x^2+y^2
    
    # a = np.ones((X.shape[0],1)) #Kernel = 1
    
    # a = np.sum(X, axis=1)  #Kernel = x+y
...
def perceptron(X, Y):
    ...
    """
    train_accuracy = calculate_accuracy(perceptron.predict(kernelX), Y)
    ...
    W = perceptron.coef_[0]                # 3D normal
    intercept = perceptron.intercept_[0]   # Distance from origin
